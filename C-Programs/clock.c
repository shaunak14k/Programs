...
int main(int argc, char* argv[])
{
    ...
    glutInitDisplayMode(GLUT_DOUBLE | GLUT_RGBA);       //Display mode : GLUT_DOUBLE & GLUT_RGBA

    glutCreateWindow("Analog Clock by Chinmay & Shaunak");      //Title of window

    glutDisplayFunc(RenderScene);       //void glutDisplayFunc(void (*func)(void));  func: The new display callback function.

    glutReshapeFunc(ChangeSize);        //void glutReshapeFunc(void (*func)(int width, int height)); func: The new reshape callback function.

    glutTimerFunc(33, TimerFunction, 1);        //void glutTimerFunc(unsigned int msecs,void (*func)(int value), value);

    SetupRC();      //Clear color window

    glutMainLoop();     //Window closes only after clicking cross "x"
